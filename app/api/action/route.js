    ...
    if (action === 'getDashboardData') {
      ...
      if (role === 'admin' || role === 'guru') {
        ...
        const history = await turso.execute({ 
          sql: "SELECT r.ResultID, r.ExamID, r.WaktuSubmit, r.TotalNilai as Nilai, e.Judul, e.AllowDownloadR, e.AllowDownloadQ FROM Results r JOIN Exams e ON r.ExamID = e.ExamID WHERE r.SiswaID = ?", 
          args: [userId] 
        });
